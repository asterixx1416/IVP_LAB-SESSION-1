...
def plot_signal():
    ...
    if not (num_points.isdigit() and start_point.isdigit()):
        messagebox.showerror("Error", "Number of data points and starting point must be integers.")
        return
    ...
    try:
        ...
        if len(values) != num_points:
            messagebox.showerror("Error", "Number of values does not match the number of data points.")
            return
    except ValueError:
        messagebox.showerror("Error", "Invalid input for values.")
        return
